...
class NoseIndex:
    ...
    def _build_index(self):
        ...
        if self.index_type == "flat":
            return faiss.IndexFlat2(d) if self.metric == "inner_product" \
    ...
    # -- enrollment (Stage 8) --------------------------------------------
    def enroll(self, dog_id: str, embeddings: list[np.ndarray]) -> EnrollmentResult:
    ...
    def identify(self, embedding: np.ndarray,
        ...
        best = cands[0]
        sim = best["similarity"]
        # confidence maps similarity [reject_thr..1] -> [0..100]
        conf = round(max(0.0, min(1.0, (sim - self.reject_thr) /
                                 (1.0 - self.reject_thr + 1e-6))) * 100, 2)
        if sim >= self.verify_thr:
            status = "Verified"
        ...
        return IdentificationResult(
            ...
            similarity=round(sim * 100, 2), confidence=conf,
    ...
    def save(self, dir_path: str) -> None:
        ...
        meta = {
            ...
            "reject_thr": self.reject_thr, "top_k": self.top_k,
