"""Smoke-test a saved anatomy-specific identity gallery.

This deliberately re-runs the complete detector -> anatomy -> feature
embedding path. It therefore catches representation drift that a crop-only
gallery test would miss.
"""
...
def evaluate_gallery(source_data: str, weights: str, index_dir: str,
                     output: str, device_value: str) -> dict:
    ...
    if not (gallery_path / "feature_meta.json").is_file():
        raise FileNotFoundError(f"feature gallery not found: {gallery_path}")
    if not Path(weights).exists():
        raise FileNotFoundError(f"embedding checkpoint not found: {weights}")
    index = FeatureIndex.load(str(gallery_path))
        ...
        for image_path, dog_id in _source_images(source_root, split) or ():
            ...
            try:
                _, _, _, features = run_to_feature_embeddings(
                    pc, cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB), dog_id=dog_id)
